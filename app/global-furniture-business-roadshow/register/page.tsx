  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(result?.error || "Failed to submit roadshow registration")
      }

      setIsSuccess(true)
    } catch (error) {
      console.error("Error submitting roadshow form:", error)
      alert(error instanceof Error ? error.message : "Unable to submit your registration right now. Please try again.")
    } finally {
